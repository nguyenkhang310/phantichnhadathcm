...
# Chay: Rscript scripts/scrapers/thu_thap_chotot.R
# Output: data/cache/cache_chotot.sqlite va data/raw/chotot/chotot_schema_chuan.csv
# ============================================================
...
# Hàm safe_worker_count: đếm hoặc kiểm tra điều kiện xử lý.
safe_worker_count <- function(requested_workers) {
  ...
  available <- suppressWarnings(as.integer(future::availableCores()))
  if (is.na(available) || available < 1L) available <- 1L
  max(1L, min(requested_workers, available))
}
...
CFG <- list(
  ...
  delay_min = 0.15,
  delay_max = 0.40,
  n_workers = safe_worker_count(Sys.getenv("CHOTOT_WORKERS", "4")),
  retry_times = 3,
  retry_delay = 2,
  sqlite_path = PATHS$chotot_sqlite,
  csv_path = PATHS$chotot_raw_csv,
  use_area_filter = identical(Sys.getenv("CHOTOT_USE_AREA_FILTER", "0"), "1")
)

DISTRICTS <- c(
  "96" = "Quận 1", "98" = "Quận 3", "99" = "Quận 4",
  "100" = "Quận 5", "101" = "Quận 6", "102" = "Quận 7",
  "103" = "Quận 8", "105" = "Quận 10", "106" = "Quận 11",
  "107" = "Quận 12", "108" = "Bình Tân", "109" = "Bình Thạnh",
  "110" = "Gò Vấp", "111" = "Phú Nhuận", "112" = "Tân Bình",
  "113" = "Tân Phú", "115" = "Bình Chánh", "116" = "Củ Chi",
  "117" = "Hóc Môn", "118" = "Nhà Bè", "119" = "Thành phố Thủ Đức",
  "120" = "Cần Giờ"
)
...
# Hàm parse_posted_at: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
# Hàm init_db: lưu hoặc cập nhật dữ liệu đầu ra.
...
# Hàm fetch_page: nạp dữ liệu từ file, API hoặc cache.
fetch_page <- function(district_id = NA_character_, district_name = NA_character_,
  ...
  # Hàm attempt: hỗ trợ xử lý dữ liệu trong script.
  attempt <- function() {
    query <- list(
      region_v2 = "13000",
      cg = category_id,
  ...
  # Hàm safe_col: đếm hoặc kiểm tra điều kiện xử lý.
...
# Hàm clean_listings: làm sạch và chuẩn hóa dữ liệu nguồn.
...
# Hàm upsert_listings: lưu hoặc cập nhật dữ liệu đầu ra.
...
# Hàm run_scrape: chạy toàn bộ bước xử lý chính.
run_scrape <- function(max_pages = CFG$max_pages) {
  dir.create(dirname(CFG$sqlite_path), recursive = TRUE, showWarnings = FALSE)
  dir.create(dirname(CFG$csv_path), recursive = TRUE, showWarnings = FALSE)
...
# Hàm refresh_data: hỗ trợ xử lý dữ liệu trong script.
