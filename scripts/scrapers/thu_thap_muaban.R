# ============================================================
# MUABAN SCRAPER - BDS TP.HCM
# Chay:
#   MUABAN_MAX_PAGES=3 Rscript scripts/scrapers/thu_thap_muaban.R
# Output:
...
# Hàm make_page_url: tạo dữ liệu phục vụ xử lý hoặc trực quan hóa.
...
# Hàm is_cloudflare_challenge: đếm hoặc kiểm tra điều kiện xử lý.
...
# Hàm render_html_chromote: hỗ trợ xử lý dữ liệu trong script.
...
# Hàm safe_text: đếm hoặc kiểm tra điều kiện xử lý.
...
# Hàm parse_area_m2: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
# Hàm parse_rooms: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
# Hàm extract_ward: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
# Hàm digest_text: hỗ trợ xử lý dữ liệu trong script.
digest_text <- function(x) {
  paste0(abs(sum(utf8ToInt(x %||% ""))), "_", nchar(x %||% ""))
}

# Hàm extract_muaban_id: phân tích chuỗi đầu vào thành giá trị chuẩn.
extract_muaban_id <- function(url) {
  id <- str_match(url %||% "", "([0-9]+)(?:\\.htm|$|\\?)")[, 2]
  if (is.na(id) || id == "") digest_text(url) else id
}

# Hàm looks_like_listing_href: hỗ trợ xử lý dữ liệu trong script.
...
# Hàm html_has_muaban_listing: hỗ trợ xử lý dữ liệu trong script.
...
# Hàm candidate_card: tạo thành phần giao diện.
...
# Hàm parse_listing_anchor: phân tích chuỗi đầu vào thành giá trị chuẩn.
parse_listing_anchor <- function(anchor, meta, page_url, page) {
  href <- normalize_url(anchor %>% html_attr("href"))
  title <- str_squish(anchor %>% html_text2())
...
# Hàm fetch_html: nạp dữ liệu từ file, API hoặc cache.
...
# Hàm fetch_source_page: nạp dữ liệu từ file, API hoặc cache.
...
# Hàm clean_muaban: làm sạch và chuẩn hóa dữ liệu nguồn.
...
# Hàm standardize_muaban_types: làm sạch và chuẩn hóa dữ liệu nguồn.
...
# Hàm run_muaban_scrape: chạy toàn bộ bước xử lý chính.
