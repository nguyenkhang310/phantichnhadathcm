...
# Chay:
#   ALONHADAT_MAX_PAGES=5 Rscript scripts/scrapers/thu_thap_alonhadat.R
# Output:
...
CFG_ALO <- list(
  ...
  output_dir = dirname(PATHS$alonhadat_raw_csv),
  csv_file = basename(PATHS$alonhadat_raw_csv)
)
...
# Hàm safe_attr: đếm hoặc kiểm tra điều kiện xử lý.
...
# Hàm extract_id: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
# Hàm extract_number: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
extract_district <- function(address) {
  if (is.na(address)) return("Không rõ")
  patterns <- c(
  ...
  ifelse(is.na(found), "Không rõ", found)
}
...
# Hàm parse_listing_node: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
# Hàm clean_alonhadat: làm sạch và chuẩn hóa dữ liệu nguồn.
...
# Hàm run_alonhadat_scrape: chạy toàn bộ bước xử lý chính.
