...
# Chay: Rscript scripts/processing/tao_dac_trung.R
# Input uu tien: data/interim/du_lieu_gop_nguon.csv, fallback: data/cache/cache_chotot.sqlite, data/raw/chotot/chotot_schema_chuan.csv
# Output: data/main/du_lieu_chinh.csv
# ============================================================
...
# Hàm read_project_data: nạp dữ liệu từ file, API hoặc cache.
read_project_data <- function() {
  ...
  stop("Chua co data. Hay chay scraper va scripts/processing/gop_nguon_du_lieu.R truoc.")
}

# Hàm haversine_km: hỗ trợ xử lý dữ liệu trong script.
...
# Hàm mode_numeric: hỗ trợ xử lý dữ liệu trong script.
...
# Hàm extract_first_number_pair: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
# Hàm extract_first_number_by_pattern: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
# Hàm has_text_pattern: đếm hoặc kiểm tra điều kiện xử lý.
has_text_pattern <- function(text, pattern) {
  as.integer(str_detect(text, pattern))
}

# Hàm build_features: làm sạch và chuẩn hóa dữ liệu nguồn.
build_features <- function(df) {
  ...
  df %>%
    ...
    select(-area_median_group, -room_mode_category, -is_rent_raw, -text_raw, -text_key)
}
