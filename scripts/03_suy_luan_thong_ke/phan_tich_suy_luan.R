...
run_statistical_inference <- function(bootstrap_reps = as.integer(Sys.getenv("BOOTSTRAP_REPS", "300"))) {
  ...
  if (nrow(bootstrap_tbl) > 0) {
    p_boot <- bootstrap_tbl %>%
      ggplot(aes(x = reorder(group, median_value), y = median_value / 1e6)) +
      ...
      theme_minimal(base_size = 12)
    ggsave(file.path(PLOT_DIR, "10_bootstrap_ci_trung_vi_gia_m2.png"), p_boot, width = 10, height = 7, dpi = 160)
  }
