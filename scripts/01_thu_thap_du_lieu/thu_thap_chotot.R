...
fetch_page <- function(district_id = NA_character_, district_name = NA_character_,
  ...
  tibble(
    ...
    title = clean_listing_title(safe_col(ads$subject)),
...
clean_listings <- function(raw_df) {
  ...
  raw_df %>%
    ...
    standardize_listing_schema(default_source = "chotot")
}
...
run_scrape <- function(max_pages = CFG$max_pages) {
  ...
  all_df <- dbReadTable(con, "listings") %>%
    ...
    standardize_listing_schema(default_source = "chotot")
  dbDisconnect(con)
  all_df <- combine_listing_history(all_df, csv_path, default_source = "chotot")
  write_listing_csv(all_df, csv_path, default_source = "chotot", keep_existing_if_empty = TRUE)
