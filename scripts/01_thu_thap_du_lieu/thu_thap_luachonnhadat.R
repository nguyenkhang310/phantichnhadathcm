...
standardize_luachon <- function(raw_df) {
  ...
  raw_df %>%
    mutate(
      ...
      tieu_de = clean_listing_title(title),
...
to_standard_schema <- function(df) {
  df %>%
    ...
    standardize_listing_schema(default_source = "luachonnhadat")
}
...
run_luachon_scrape <- function(max_pages = CFG_LCN$max_pages) {
  ...
  if (nrow(raw) == 0) {
    if (file.exists(standard_out)) {
      old <- read_csv(standard_out, show_col_types = FALSE) %>%
        standardize_listing_schema(default_source = "luachonnhadat")
      message("Khong co tin Luachonnhadat moi; giu nguyen ", nrow(old), " dong trong ", standard_out)
  ...
  if (nrow(clean) == 0) {
    if (file.exists(standard_out)) {
      old <- read_csv(standard_out, show_col_types = FALSE) %>%
        standardize_listing_schema(default_source = "luachonnhadat")
      message("Luachonnhadat khong co dong moi sau clean; giu nguyen ", nrow(old), " dong trong ", standard_out)
      return(invisible(old))
    }
  ...
  standard_df <- to_standard_schema(clean) %>%
    combine_listing_history(standard_out, default_source = "luachonnhadat")

  write_csv(assignment_df, clean_out)
  write_listing_csv(standard_df, standard_out, default_source = "luachonnhadat", keep_existing_if_empty = TRUE)
