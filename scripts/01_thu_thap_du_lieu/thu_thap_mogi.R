...
CFG_MOGI <- list(
  ...
  max_pages = as.integer(Sys.getenv("MOGI_MAX_PAGES", "5")),
...
run_mogi_scrape <- function(
  ...
  if (nrow(clean) == 0) {
    if (file.exists(output_csv)) {
      old <- read_csv(output_csv, show_col_types = FALSE) %>%
        format_mogi_output()
      message("Khong co tin Mogi moi; giu nguyen ", nrow(old), " dong trong ", output_csv)
  ...
  clean <- clean %>%
    mutate(
      tieu_de = clean_listing_title(tieu_de),
