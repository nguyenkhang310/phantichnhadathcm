...
parse_listing_node <- function(node, meta, page_url, page) {
  ...
  title <- clean_listing_title(safe_text(node, ".property-title"))
...
clean_alonhadat <- function(df) {
  ...
  df %>%
    ...
    standardize_listing_schema(default_source = "alonhadat")
}
  ...
  if (nrow(clean) == 0) {
    if (file.exists(out)) {
      old <- read_csv(out, show_col_types = FALSE) %>%
        standardize_listing_schema(default_source = "alonhadat")
      message("Khong co tin Alonhadat moi hop le; giu nguyen ", nrow(old), " dong trong ", out)
      return(invisible(old))
    }
    stop("Khong lay duoc listing Alonhadat hop le. Kiem tra ket noi, selector hoac website.")
  }

  clean <- combine_listing_history(clean, out, default_source = "alonhadat")
  write_listing_csv(clean, out, default_source = "alonhadat", keep_existing_if_empty = TRUE)
