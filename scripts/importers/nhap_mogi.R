# ============================================================
# IMPORT MOGI CLEAN CSV - Chuan hoa vao schema noi bo
# Chay:
#   Rscript scripts/importers/nhap_mogi.R
# Input:
...
# Hàm parse_mogi_price_one: phân tích chuỗi đầu vào thành giá trị chuẩn.
parse_mogi_price_one <- function(gia_raw, gia_vnd) {
  ...
  # Hàm to_num: hỗ trợ xử lý dữ liệu trong script.
...
# Hàm parse_mogi_price: phân tích chuỗi đầu vào thành giá trị chuẩn.
parse_mogi_price <- function(gia_raw, gia_vnd) {
  mapply(parse_mogi_price_one, gia_raw, gia_vnd, USE.NAMES = FALSE)
}

# Hàm parse_mogi_date: phân tích chuỗi đầu vào thành giá trị chuẩn.
...
# Hàm category_id_from_name: hỗ trợ xử lý dữ liệu trong script.
...
# Hàm standardize_mogi: làm sạch và chuẩn hóa dữ liệu nguồn.
standardize_mogi <- function(df) {
  ...
  df %>%
    mutate(
      ...
      title = str_squish(as.character(tieu_de)),
...
# Hàm read_mogi_source: nạp dữ liệu từ file, API hoặc cache.
read_mogi_source <- function(path, priority) {
  read_csv(path, col_types = cols(.default = col_character())) %>%
    mutate(.source_priority = priority)
}

# Hàm run_import_mogi: chạy toàn bộ bước xử lý chính.
